#!/bin/bash
# cap-ipaws-bridge install helper script
...
if [[ $NOPE -eq 1 ]]; then
    ...
    sudo systemctl stop cap_alert_bridge || true
    sudo systemctl disable cap_alert_bridge || true

    sudo systemctl stop pong_bot || true
    sudo systemctl disable pong_bot || true

    sudo systemctl stop cap_alert_bridge_w3_server || true
    sudo systemctl disable cap_alert_bridge_w3_server || true

    sudo systemctl stop cap_alert_bridge_reporting || true
    sudo systemctl disable cap_alert_bridge_reporting || true

    sudo rm -f /etc/systemd/system/cap_alert_bridge.service
    sudo rm -f /etc/systemd/system/cap_alert_bridge_reporting
    sudo rm -f /etc/systemd/system/pong_bot.service
    sudo rm -f /etc/systemd/system/cap_alert_bridge_w3_server.service
    sudo rm -f /etc/systemd/system/cap_alert_bridge_reporting.service
    sudo rm -f /etc/systemd/system/cap_alert_bridge_reporting.timer
    ...
    sudo rm -rf /opt/cap-ipaws-bridge/
...
# check if we are in /opt/cap-ipaws-bridge
if [[ "$program_path" != "/opt/cap-ipaws-bridge" ]]; then
    echo "----------------------------------------------"
    echo "  Project Path Decision"
    echo "----------------------------------------------"
    printf "\nIt is recommended to install Meshing Around in /opt/cap-ipaws-bridge if used as a service.\n"
    printf "Do you want to move the project to /opt/cap-ipaws-bridge now? (y/n): "
    read move
    if [[ $(echo "$move" | grep -i "^y") ]]; then
        sudo mv "$program_path" /opt/cap-ipaws-bridge
        cd /opt/cap-ipaws-bridge
        sudo git config --global --add safe.directory /opt/cap-ipaws-bridge
        printf "\nProject moved to /opt/cap-ipaws-bridge.\n"
...
cp etc/cap_alert_bridge.tmp etc/cap_alert_bridge.service
cp etc/cap_alert_bridge_reporting.tmp etc/cap_alert_bridge_reporting.service
cp etc/cap_alert_bridge_w3_server.tmp etc/cap_alert_bridge_w3_server.service

# set the correct path in the service file
replace="s|/dir/|$program_path/|g"
sed -i "$replace" etc/pong_bot.service
sed -i "$replace" etc/cap_alert_bridge.service
sed -i "$replace" etc/cap_alert_bridge_reporting.service
sed -i "$replace" etc/cap_alert_bridge_w3_server.service
...
if [[ $(echo "${embedded}" | grep -i "^y") ]]; then
    ...
    if [[ $(echo "${venv}" | grep -i "^y") ]]; then
        ...
        if ! python3 -m venv --help &> /dev/null; then
            ...
            replace="s|python3 cap_alert_bridge.py|/usr/bin/bash launch.sh mesh|g"
            sed -i "$replace" etc/cap_alert_bridge.service
...
sed -i "$replace" etc/cap_alert_bridge.service
sed -i "$replace" etc/cap_alert_bridge_reporting.service
sed -i "$replace" etc/cap_alert_bridge_reporting.timer
...
if [[ $(echo "${bot}" | grep -i "^m") ]]; then
    ...
    sudo cp etc/cap_alert_bridge.service /etc/systemd/system/
    sudo systemctl enable cap_alert_bridge.service
    sudo systemctl daemon-reload
    echo "to start mesh bot service: systemctl start cap_alert_bridge"
    service="cap_alert_bridge"
fi

# install cap_alert_bridge_reporting timer to run daily at 4:20 am
echo ""
echo "Installing cap_alert_bridge_reporting.timer to run cap_alert_bridge_reporting daily at 4:20 am..."
sudo cp etc/cap_alert_bridge_reporting.service /etc/systemd/system/
sudo cp etc/cap_alert_bridge_reporting.timer /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable cap_alert_bridge_reporting.timer
sudo systemctl start cap_alert_bridge_reporting.timer
echo "cap_alert_bridge_reporting.timer installed and enabled"
echo "Check timer status with: systemctl status cap_alert_bridge_reporting.timer"
echo "List all timers with: systemctl list-timers"
echo ""

# # install cap_alert_bridge_w3_server service
# echo "Installing cap_alert_bridge_w3_server.service to run the web3 server..."
# sudo cp etc/cap_alert_bridge_w3_server.service /etc/systemd/system/
# sudo systemctl daemon-reload
# sudo systemctl enable cap_alert_bridge_w3_server.service
# sudo systemctl start cap_alert_bridge_w3_server.service
# echo "cap_alert_bridge_w3_server.service installed and enabled"
# echo "Check service status with: systemctl status cap_alert_bridge_w3_server.service"
# echo ""
...
if [[ $(echo "${embedded}" | grep -i "^n") ]]; then
    ...
    printf "\ncap_alert_bridge_reporting.timer installed to run daily at 4:20 am\n" >> install_notes.txt
    printf "Check timer status: systemctl status cap_alert_bridge_reporting.timer\n" >> install_notes.txt
    printf "List all timers: systemctl list-timers\n" >> install_notes.txt
    printf "View timer logs: journalctl -u cap_alert_bridge_reporting.timer\n" >> install_notes.txt
    printf "*** Stay Up to date using 'bash update.sh' ***\n" >> install_notes.txt
    printf "sudo ./update.sh && sudo -u meshbot ./launch.sh cap_alert_bridge.py\n" >> install_notes.txt
...
# sudo systemctl stop cap_alert_bridge
# sudo systemctl disable cap_alert_bridge

# sudo systemctl stop pong_bot
# sudo systemctl disable pong_bot

# sudo systemctl stop cap_alert_bridge_w3_server
# sudo systemctl disable cap_alert_bridge_w3_server

# sudo systemctl stop cap_alert_bridge_reporting
# sudo systemctl disable cap_alert_bridge_reporting

# sudo rm /etc/systemd/system/cap_alert_bridge.service
# sudo rm /etc/systemd/system/cap_alert_bridge_reporting
# sudo rm /etc/systemd/system/pong_bot.service
# sudo rm /etc/systemd/system/cap_alert_bridge_w3_server.service
# sudo rm /etc/systemd/system/cap_alert_bridge_reporting.service
# sudo rm /etc/systemd/system/cap_alert_bridge_reporting.timer
