...
# Install cap_alert_bridge as a systemd service for the current user.
# Defaults:
#   - project path: /opt/cap-ipaws-bridge
#   - service name: cap_alert_bridge
#   - service user: invoking user (SUDO_USER when using sudo)

SERVICE_NAME="cap_alert_bridge"
PROJECT_PATH="/opt/cap-ipaws-bridge"
...
Options:
  --project-path PATH   Project root path (default: /opt/cap-ipaws-bridge)
  ...
  --direct-python       Run python3 cap_alert_bridge.py directly (skip launch.sh)
...
Examples:
  ...
  sudo bash etc/install_service.sh --project-path /opt/cap-ipaws-bridge --user $USER
EOF
}
...
[[ -f "$PROJECT_PATH/cap_alert_bridge.py" ]] || die "cap_alert_bridge.py not found in $PROJECT_PATH"
...
if [[ $USE_LAUNCH_SH -eq 1 ]]; then
    ...
    EXEC_START="/usr/bin/python3 $PROJECT_PATH/cap_alert_bridge.py"
fi
