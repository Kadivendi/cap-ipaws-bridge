...
for dir in "/opt/cap-ipaws-bridge" "/opt/cap-ipaws-bridge/logs" "/opt/cap-ipaws-bridge/data"; do
  if [ -d "$dir" ]; then
...
if [ -f "/opt/cap-ipaws-bridge/config.ini" ]; then
  chown "$TARGET_USER:$TARGET_USER" "/opt/cap-ipaws-bridge/config.ini"
  chmod 664 "/opt/cap-ipaws-bridge/config.ini"
else
  echo "Warning: /opt/cap-ipaws-bridge/config.ini does not exist, skipping."
fi
