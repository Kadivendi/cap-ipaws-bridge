...
This document covers the Bulliten Board System or BBS componment of the cap-ipaws-bridge project.
...
- **How-To:**
  ...
  2. **Copy Files:**  
     ...
     ```sh
     scp user@remote:/path/to/cap-ipaws-bridge/data/bbsdb.pkl ./data/bbsdb.pkl
     scp user@remote:/path/to/cap-ipaws-bridge/data/bbsdm.pkl ./data/bbsdm.pkl
     ```
  3. **Reload Database:**  
...
schedule.every(2).days.at("10:00").do(send_message("bbslink CapAlertBridge looking for peers", schedulerChannel, 0, schedulerInterface))
```
...
`.do(send_message("bbslink CapAlertBridge looking for peers", schedulerChannel, 0, schedulerInterface))`
