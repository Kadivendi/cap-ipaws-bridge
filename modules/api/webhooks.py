...
@webhook_router.post("")
def register_webhook(payload: RegisterPayload) -> dict:
    ep = WebhookEndpoint(
        id=uuid.uuid4().hex[:12],
        url=payload.url,
        secret=payload.secret,
        events=payload.events or ["alert.new", "alert.update", "alert.cancel"],
    )
    try:
        _dispatcher.register(ep)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return {"id": ep.id, "events": ep.events}


@webhook_router.delete("/{webhook_id}")
def deregister_webhook(webhook_id: str) -> dict:
    if not _dispatcher.deregister(webhook_id):
        raise HTTPException(status_code=404, detail="Webhook not found")
    return {"id": webhook_id, "removed": True}
...
def get_dispatcher() -> WebhookDispatcher:
    """Expose the singleton so other modules can fire events."""
    return _dispatcher
