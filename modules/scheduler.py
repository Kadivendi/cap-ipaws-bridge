# modules/scheduler.py 2025 cap-ipaws-bridge
# Scheduler module for cap_alert_bridge
import asyncio
import schedule
...
def setup_scheduler(
    ...
    try:
        # Methods imported from cap_alert_bridge for scheduling tasks
        from cap_alert_bridge import (
            tell_joke,
            welcome_message,
            handle_wxc,
            handle_moon,
        ...
        logger.warning(f"Some cap_alert_bridge schedule features are unavailable by option disable in config.ini: {e} comment out the use of these methods in your custom_scheduler.py")
    ...
    try:
        ...
        if any(option in schedulerValue for option in basicOptions):
            ...
            schedule.every(schedulerIntervalInt).hours.do(
                lambda: send_message("bbslink CapAlertBridge looking for peers", schedulerChannel, 0, schedulerInterface)
            )
