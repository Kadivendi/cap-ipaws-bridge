...
# SMTP Configuration
if enableSMTP:
    from modules.smtp import * # from the spudgunman/cap-ipaws-bridge repo
...
if solar_conditions_enabled:
    from modules.space import * # from the spudgunman/cap-ipaws-bridge repo
...
if location_enabled:
    from modules.locationdata import * # from the spudgunman/cap-ipaws-bridge repo
    ...
    if enableGBalerts and not enableDEalerts:
        from modules.globalalert import * # from the spudgunman/cap-ipaws-bridge repo
    ...
    if use_meteo_wxApi:
        ...
        from modules.wx_meteo import * # from the spudgunman/cap-ipaws-bridge repo
    else:
        # NOAA only features
...
# BBS Configuration
if bbs_enabled:
    from modules.bbstools import * # from the spudgunman/cap-ipaws-bridge repo
...
if dad_jokes_enabled:
    from modules.games.joke import * # from the spudgunman/cap-ipaws-bridge repo
...
# LLM Configuration
if llm_enabled:
    from modules.llm import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + trap_list_llm # items ask:
    help_message = help_message + ", askai"

# DopeWars Configuration
if dopewars_enabled:
    from modules.games.dopewar import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + ("dopewars",)
    games_enabled = True

# Lemonade Stand Configuration
if lemonade_enabled:
    from modules.games.lemonade import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + ("lemonstand",)
    games_enabled = True

# BlackJack Configuration
if blackjack_enabled:
    from modules.games.blackjack import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + ("blackjack",)
    games_enabled = True

# Video Poker Configuration
if videoPoker_enabled:
    from modules.games.videopoker import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + ("videopoker",)
    games_enabled = True

if mastermind_enabled:
    from modules.games.mmind import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + ("mastermind",)
    games_enabled = True

if golfSim_enabled:
    from modules.games.golfsim import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + ("golfsim",)
    games_enabled = True

if hangman_enabled:
    from modules.games.hangman import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + ("hangman",)
    games_enabled = True

if hamtest_enabled:
    from modules.games.hamtest import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + ("hamtest",)
    games_enabled = True

if tictactoe_enabled:
    from modules.games.tictactoe import TicTacToe # from the spudgunman/cap-ipaws-bridge repo
    tictactoe = TicTacToe(display_module=None)
    trap_list = trap_list + ("tictactoe","tic-tac-toe",)

if quiz_enabled:
    from modules.games.quiz import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + trap_list_quiz # items quiz, q:
    help_message = help_message + ", quiz"
    # games not enabled for quiz

if survey_enabled:
    from modules.survey import * # from the spudgunman/cap-ipaws-bridge repo
    trap_list = trap_list + trap_list_survey # items survey, s:
    help_message = help_message + ", survey"
    games_enabled = True

if wordOfTheDay:
    from modules.games.wodt import WordOfTheDayGame # from the spudgunman/cap-ipaws-bridge repo
    theWordOfTheDay = WordOfTheDayGame()
    # this runs in background and wont enable other games

if battleship_enabled:
    from modules.games.battleship import playBattleship # from the spudgunman/cap-ipaws-bridge repo
...
if qrz_hello_enabled:
    from modules.qrz import * # from the spudgunman/cap-ipaws-bridge repo
    #trap_list = trap_list + trap_list_qrz # items qrz, qrz?, qrzcall
    #help_message = help_message + ", qrz"

# CheckList Configuration
if checklist_enabled:
    from modules.checklist import * # from the spudgunman/cap-ipaws-bridge repo
...
if inventory_enabled:
    from modules.inventory import * # from the spudgunman/cap-ipaws-bridge repo
...
if file_monitor_enabled or read_news_enabled or bee_enabled or enable_runShellCmd or cmdShellSentryAlerts:
    from modules.filemon import * # from the spudgunman/cap-ipaws-bridge repo
