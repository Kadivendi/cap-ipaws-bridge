...
def draw_board(screen, board, meta=None):
    ...
    title_text = title_font.render("CapAlertBridge Tic-Tac-Toe", True, (220, 220, 255))
...
def ttt_main(fullscreen=True):
    ...
    print(f"[CapAlertBridge TTT Display] Pygame version: {pygame.version.ver}")
    print(f"[CapAlertBridge TTT Display] Resolution: {info.current_w}x{info.current_h} ({mode})")
    print(f"[CapAlertBridge TTT Display] Display driver: {pygame.display.get_driver()}")
    running = True
    while running:
